...
func secFromBytes(dst, src string, in []byte) (gopass.Secret, error) {
	...
	// TODO: Should use base64.NewEncoder to avoid expensive allocations on potentially
	// large blobs.
	n, err := sec.Write([]byte(base64.StdEncoding.EncodeToString(in)))
	if err != nil {
		debug.Log("Failed to write to secret: %v", err)

		return sec, err
	}
	debug.Log("Wrote %d bytes of Base64 encoded bytes to secret", n)
...
func (s *Action) binaryCopyFromFileToStore(ctx context.Context, from, to string, deleteSource bool) error {
	...
	if err != nil {
		return fmt.Errorf("failed to parse secret from input: %v", err)
	}
	if err := s.Store.Set(
