...
func fetchAliases() (map[string][]string, error) {
	...
	for _, as := range ja {
		for _, a := range as {
			aliases[a] = make([]string, 0, len(as)-1)
			for _, ia := range as {
				if ia == a {
					continue
				}
				aliases[a] = append(aliases[a], ia)
			}
		}
	}
	return aliases, nil
...
func fetchRules() (map[string]rule, error) {
	...
	rules := make(map[string]rule, len(jr))
	for k, v := range jr {
		_ = v
		rules[k] = rule{} // TODO
	}
...
var pkgTpl = template.Must(template.New("").Parse(`// Generated by go generated. DO NOT EDIT
